...
```{r pivot_creation, echo=FALSE}
# Add a new column with info on the sample
PHA1 <- PHA1 %>%
  add_column (sample = "PHA1") %>%
  select (ARG, normalized_count, sample)

# To create an example: modify the ARG column so it's not exactly the same but in a way that can be traced back
PHA2 <- PHA1 %>% 
  mutate(ARG = sprintf("ID_%s", ARG)) %>%
  mutate(sample = sub('PHA1', 'PHA2', sample)) %>%
...
# union_all() combines all rows from both the tables **without** removing the duplicate records from the combined dataset
  union(PHA1) %>%
# Join the two columns with merge() (all=TRUE -> outer join)
# merge() two or more data frames as argument with an option all=TRUE as shown below, which finds union of the dataframes in R
# merge (y=PHA1, by=c("ARG","normalized_count","sample"), all=TRUE) %>%
...
#* Use `values_fn = {summary_fun}` to summarise duplicatesError: Can't convert <double> to <list>.
    )
# which(duplicated(PHA2))
# %>% write_tsv(path = "pivot.16S_normalized.tsv", col_names = TRUE)
```
